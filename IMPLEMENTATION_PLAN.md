...
- [ ] Set web env vars in Vercel: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_API_BASE_URL` (→ the prod API origin), `VITE_SENTRY_DSN`, `VITE_POSTHOG_KEY`, `VITE_POSTHOG_HOST`. *(Dashboard step.)* **⚠️ THIS IS THE ACTIVE BLOCKER (verified live 2026-07-11):** builds are green (latest 8 deploys `READY`, auto-deploy from `main` works, prod domain `softball-app-one.vercel.app`), but the live site throws `Missing required Vite environment variable: supabaseUrl` in the browser console → blank app until these are set. Build success does NOT imply env is set — the guard is a runtime check in `lib/env.ts`. Copy `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY`/`VITE_POSTHOG_*` from `apps/web/.env`; `VITE_API_BASE_URL` must wait for the Render URL (§5.5.2) and end in `/api`.
