...
>
> **Last updated:** 2026-06-05 (Phase 5.5 — deploy code artifacts landed: `apps/web/vercel.json` SPA rewrite, root `render.yaml` API blueprint, `.github/workflows/ci.yml` lint+typecheck, and a `PORT`-binding fix so the API works behind Render's proxy. Remaining 5.5 work is dashboard-only: create the Vercel + Render projects, set env vars, prod OAuth, and the live smoke test.)
...
- [ ] Create a Vercel project from the repo. Root directory `apps/web`, framework preset **Vite**, build `npm run build`, output `dist`. *(Dashboard step — runbook below.)* **⚠️ MISCONFIGURED (found 2026-07-09): the `softball-app` Vercel project exists but its Root Directory is pointed at `apps/api` and Framework = `express`, so every push builds the backend (`@softball/api` → `tsc`) and all 14 deployments have errored on `src/app.ts(31,11) TS2349: helmet has no call signatures`. That helmet error is NOT a code bug — the API builds clean locally (`npm run build -w @softball/api` → exit 0) and CI typecheck is green; it only surfaces because Vercel does a fresh isolated install of the wrong workspace. FIX (dashboard only, can't be done from code): Settings → General → Root Directory `apps/api` → `apps/web`; Framework Preset `Express` → `Vite` (auto-detects once rooted at `apps/web` and reads `apps/web/vercel.json`); then redeploy. The repo's `apps/web/vercel.json` is already correct — it just wasn't being read.**
...
- [ ] Set web env vars in Vercel: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_API_BASE_URL` (→ the prod API origin), `VITE_SENTRY_DSN`, `VITE_POSTHOG_KEY`, `VITE_POSTHOG_HOST`. *(Dashboard step.)*
...
- [ ] Set all API env vars (the `sync: false` keys in `render.yaml`): `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `ENCRYPTION_KEY`, `GOOGLE_OAUTH_CLIENT_ID/SECRET/REDIRECT_URI`, `RESEND_API_KEY`, `EMAIL_FROM`, `SENTRY_DSN`. *(Dashboard step.)*
